...
TF-IDF was selected as the retrieval method to establish a baseline for the document pipeline. In an industrial knowledge base context, TF-IDF provides clear insight into how term frequency, document rarity, and chunking decisions affect retrieval outcomes. This made it well-suited for validating data ingestion, chunking logic, schema consistency, and evaluation metrics before introducing more complex semantic models. Using TF-IDF also exposed known limitations—such as sensitivity to generic language and inability to infer user intent—which were leveraged to perform meaningful error analysis and motivate future improvements. 
